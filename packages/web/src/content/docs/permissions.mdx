...
By default, OpenSploit allows most operations without approval, except `doom_loop` and `external_directory` which default to `ask`. You can configure this using the `permission` option.
...
```json title="opensploit.json" {3-7,10-14}
{
  "$schema": "https://opensploit.ai/config.json",
  "permission": {
    "bash": {
