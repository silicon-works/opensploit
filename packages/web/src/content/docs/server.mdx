...
When you run `opensploit` it starts a TUI and a server. Where the TUI is the
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenSploit [IDE](/docs/ide) plugins.
