...
There are two types of agents in OpenSploit; primary agents and subagents.
...
OpenSploit comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenSploit comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenSploit comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `opensploit.json` config file:
...
- Global: `~/.config/opensploit/agent/`
- Per-project: `.opencode/agent/`

```markdown title="~/.config/opensploit/agent/review.md"
---
...
If no temperature is specified, OpenSploit uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenSploit config and the project specific config.
...
The model ID in your OpenSploit config uses the format `provider/model-id`. For example, if you're using [OpenSploit Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="opensploit.json" {3-6,9-12}
{
  "$schema": "https://opensploit.ai/config.json",
  "tools": {
    "write": true,
...
```json title="opensploit.json" {3-5,8-10}
{
  "$schema": "https://opensploit.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="opensploit.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/silicon-works/opensploit).
:::
...
```markdown title="~/.config/opensploit/agent/docs-writer.md"
---
...
```markdown title="~/.config/opensploit/agent/security-auditor.md"
---
