...
You can add external tools to OpenSploit using the _Model Context Protocol_, or MCP.

OpenSploit supports both:
...
You can define MCP servers in your OpenSploit config under `mcp`. Add each MCP
with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="opensploit.jsonc" {6}
{
...
```jsonc title="opensploit.jsonc" {15}
{
  "$schema": "https://opensploit.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenSploit automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenSploit will:
...
If the server requires authentication, OpenSploit will prompt you to authenticate when you first try to use it.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenSploit will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.

---

## Manage

Your MCPs are available as tools in OpenSploit, alongside built-in tools. So you
can manage them through the OpenSploit config like any other tool.
...
```json title="opensploit.json" {11, 14-18}
{
  "$schema": "https://opensploit.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="opensploit.json" {4-7}
{
  "$schema": "https://opensploit.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="opensploit.json" {7-9}
{
  "$schema": "https://opensploit.ai/config.json",
  "mcp": {
    "context7": {
