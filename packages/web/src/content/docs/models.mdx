...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start OpenSploit.
...
Here are several models that work well with OpenSploit, in no particular order. (This is not an exhaustive list):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [OpenSploit Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="opensploit.jsonc" {7-12,19-24}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="opensploit.jsonc" {6-20}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "opencode": {
...
When OpenSploit starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the OpenSploit config.
