...
You can also have global rules in a `~/.config/opensploit/AGENTS.md` file. This gets applied across all opencode sessions.
...
2. **Global file** by checking `~/.config/opensploit/AGENTS.md`
...
You can specify custom instruction files in your `opensploit.json` or the global `~/.config/opensploit/opensploit.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
### Using opensploit.json

The recommended approach is to use the `instructions` field in `opensploit.json`:
...
:::tip
For monorepos or projects with shared standards, using `opensploit.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
