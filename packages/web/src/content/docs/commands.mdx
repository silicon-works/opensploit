...
You can add custom commands through the OpenSploit config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your OpenSploit [config](/docs/config):

```json title="opensploit.jsonc" {4-12}
{
...
- Global: `~/.config/opensploit/command/`
- Per-project: `.opencode/command/`

```markdown title="~/.config/opensploit/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
