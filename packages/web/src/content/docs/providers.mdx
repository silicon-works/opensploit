---
title: Providers
description: Using any LLM provider in OpenSploit.
---

import config from "../../../config.mjs"
export const console = config.console

OpenSploit uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenSploit config.
...
You can customize the providers through the `provider` section in your OpenSploit
config.
...
OpenSploit Zen is a list of models provided by the OpenSploit team that have been
tested and verified to work well with OpenSploit. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenSploit Zen.
:::

1. Run the `/connect` command in the TUI, select opencode, and head to [opensploit.ai/auth](https://opensploit.ai/auth).
...
It works like any other provider in OpenSploit. And is completely optional to use
it.
...
To use Amazon Bedrock with OpenSploit:
...
To use Google Vertex AI with OpenSploit:
...
```jsonc title="~/.config/opensploit/opensploit.jsonc"
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenSploit conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenSploit conversation listed as a separate session.
...
```json title="opensploit.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="opensploit.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "lmstudio": {
...
```json title="opensploit.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with OpenSploit:
...
3. Copy the API key for use in OpenSploit.
...
6. **Important**: Before using cloud models in OpenSploit, you must pull the model information locally:
...
OpenSploit Zen is a list of tested and verified models provided by the OpenSploit team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenSploit Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenSploit Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenSploit API key.
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in opensploit.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `opensploit.json` file in your project directory:

   ```json title="opensploit.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opensploit.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="opensploit.json" {9,11,17-20}
{
  "$schema": "https://opensploit.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow OpenSploit to understand how much context you have left. Standard providers pull these from models.dev automatically.
