...
With OpenSploit you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenSploit uses our own `opensploit` theme.
...
OpenSploit comes with several built-in themes.
...
- Want OpenSploit to match their terminal's appearance
...
OpenSploit supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/opensploit/themes/*.json` or `$XDG_CONFIG_HOME/opencode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/opensploit/themes
vim ~/.config/opensploit/themes/my-theme.json
```
...
{
  "$schema": "https://opensploit.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
