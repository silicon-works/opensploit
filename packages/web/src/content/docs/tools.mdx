...
Tools allow the LLM to perform actions in your codebase. OpenSploit comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
```markdown title="~/.config/opensploit/agent/readonly.md"
---
...
Here are all the built-in tools available in OpenSploit.
