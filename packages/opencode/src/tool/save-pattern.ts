...
const log = Log.create({ service: "tool.save-pattern" })
...
export const SavePatternTool = Tool.define("save_pattern", {
  ...
  async execute(params, ctx) {
    ...
    if (result.success && result.pattern) {
      ...
      output = [
        ...
        "",
        "**Key Insights:**",
        ...(pattern.methodology.key_insights.map((i) => `- ${i}`) || ["- None recorded"]),
        "",
