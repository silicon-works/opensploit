...
export const HostsTool = Tool.define("hosts", {
    ...
    switch (action) {
      case "add": {
        if (!entries || entries.length === 0) {
          return {
            output: "Error: No entries provided. Please specify entries with ip and hostname.",
            title: "Hosts: Error",
      ...
      case "remove": {
        if (!entries || entries.length === 0) {
          return {
            output: "Error: No entries provided. Use 'cleanup' action to remove all session entries.",
            title: "Hosts: Error",
            metadata: { success: false },
          }
        }
        ...
        if (!result.success) {
          return {
            output: `Failed to remove hosts entries: ${result.error}`,
            title: "Hosts: Failed",
            metadata: { success: false },
          }
        }
        ...
        return {
          output: "Successfully removed session hosts entries.",
      ...
      case "list": {
        ...
        if (sessionEntries.length === 0) {
          return {
            output: "No hosts entries for this session.",
        ...
        return {
          output: `Current hosts entries for this session:\n${entriesStr}`,
      ...
      case "cleanup": {
        ...
        if (!result.success) {
          return {
            output: `Failed to cleanup hosts entries: ${result.error}`,
        ...
        return {
          output: "Successfully cleaned up all hosts entries for this session.",
