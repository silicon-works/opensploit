...
const log = Log.create({ service: "tool.browser-headed" })
...
export const BrowserHeadedTool = Tool.define("browser_headed_mode", {
  ...
  async execute(params): Promise<{ output: string; title: string; metadata: { success: boolean; mode: string; vnc_url?: string } }> {
    const { enable, vpn_target, vpn_hostname, vpn_https } = params
