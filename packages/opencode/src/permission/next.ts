...
import { Ultrasploit } from "./ultrasploit"
...
export namespace PermissionNext {
  ...
  export const ask = fn(S.AskInput, async (input) => {
    const rootSessionID = getRootSession(input.sessionID) as SessionID
    if (Ultrasploit.isEnabled(rootSessionID)) {
      return
    }
    return runPromise((service) => service.ask({ ...input, sessionID: rootSessionID }))
  })
