...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opensploit and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenSploit")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opensploit-ai",
      pnpm: "pnpm uninstall -g opensploit-ai",
      bun: "bun remove -g opensploit-ai",
      yarn: "yarn global remove opensploit-ai",
      brew: "brew uninstall opensploit",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opensploit") || binDir.includes(".opensploit")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenSploit!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opensploit") || content.includes(".opensploit/bin") || content.includes("# opensploit") || content.includes(".opensploit/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# opensploit" || trimmed === "# opensploit") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opensploit/bin") || trimmed.includes(".opensploit/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".opensploit/bin") || trimmed.includes(".opensploit/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".opensploit") || trimmed.includes(".opensploit")))
    ) {
      continue
    }
