...
const AGENT_USERNAME = "opensploit-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/opensploit.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/os summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://opensploit.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/opensploit-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.opensploit.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  opensploit:
    if: |
      contains(github.event.comment.body, ' /os') ||
      startsWith(github.event.comment.body, '/os') ||
      contains(github.event.comment.body, ' /opensploit') ||
      startsWith(github.event.comment.body, '/opensploit')
    ...
    steps:
      ...
      - name: Run opensploit
        uses: silicon-works/opensploit/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.opensploit.ai" : "https://opensploit.ai"
      ...
      try {
        ...
        console.log("opensploit session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.opensploit.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/opensploit,/os")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to opensploit...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("opensploit-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule") {
        ...
        if (type === "schedule") {
          ...
          return `opensploit/scheduled-${hex}-${timestamp}`
        }
        return `opensploit/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/opensploit-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[opensploit session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the opensploit infrastructure after your response",
