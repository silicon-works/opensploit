    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (Object.keys(mcpServers).length === 0) {
          ...
          prompts.outro("Add servers with: opensploit mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in opensploit.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  async handler() {
    ...
    if (type === "local") {
      const command = await prompts.text({
        ...
        placeholder: "e.g., opensploit x @modelcontextprotocol/server-filesystem",
        validate: (x) => (x && x.length > 0 ? undefined : "Required"),
      })
    ...
    if (type === "remote") {
      ...
      if (useOAuth) {
        ...
        if (hasClientId) {
          ...
          prompts.log.info("Add this to your opensploit.json:")
          prompts.log.info(`
  "mcp": {
    "${name}": {
      "type": "remote",
