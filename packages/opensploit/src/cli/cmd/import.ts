...
export const ImportCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs.positional("file", {
      describe: "path to JSON file or opensploit.ai share URL",
      type: "string",
      demandOption: true,
    })
  },
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        const urlMatch = args.file.match(/https?:\/\/opensploit\.ai\/s\/([a-zA-Z0-9_-]+)/)
        if (!urlMatch) {
          process.stdout.write(`Invalid URL format. Expected: https://opensploit.ai/s/<slug>`)
          process.stdout.write(EOL)
          return
        }

        const slug = urlMatch[1]
        const response = await fetch(`https://api.opensploit.ai/share_data?id=${slug}`)
