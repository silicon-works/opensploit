...
if (!Script.preview) {
  ...
  const binaryPkgbuild = [
    ...
    "",
    "package() {",
    '  install -Dm755 ./opensploit "${pkgdir}/usr/bin/opensploit"',
    "}",
    "",
  ].join("\n")

  // Source-based PKGBUILD for opensploit
  const sourcePkgbuild = [
    ...
    "    fi",
    "  fi",
    '  bin="dist/opensploit-linux-${target_arch}${base}${libc}/bin/opensploit"',
    '  if [ ! -f "$bin" ]; then',
    '    printf "unable to find binary for %s%s%s\\n" "$target_arch" "$base" "$libc" >&2',
    "    return 1",
    "  fi",
    '  install -Dm755 "$bin" "${pkgdir}/usr/bin/opensploit"',
    "}",
    "",
  ].join("\n")

  for (const [pkg, pkgbuild] of [
    ["opensploit-bin", binaryPkgbuild],
  ...
  await Bun.file("./dist/homebrew-tap/opensploit.rb").write(homebrewFormula)
