...
test("discovers skills from .opensploit/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "test-skill")
      await Bun.write(
...
test("discovers multiple skills from .opensploit/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "my-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: my-skill
...
test("throws error for invalid skill name format", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "InvalidName")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: InvalidName
...
test("throws error when name doesn't match directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "dir-name")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: different-name
...
test("throws error for missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("parses optional fields", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "full-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: full-skill
...
test("ignores unknown frontmatter fields", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "extra-fields")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: extra-fields
...
test("SystemPrompt.skills() returns XML block with skills", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".opensploit", "skill", "example-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: example-skill
